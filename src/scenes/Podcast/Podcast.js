...
class Podcast extends React.Component {
  ...
  clickHandlerPodcastLater(event, _podcast){
    ...
    ? Lists.saveToList('podcasts','later',_podcast)
  ...
  clickHandlerPodcastFav(event, _podcast){
    event.stopPropagation();
    this.props.auth.isAuthenticated
    ? Lists.saveToList('podcasts','fav',_podcast)
    : localStorage.getItem('app')
      ? (
          this.props.auth.afterRequiredApp = function(){
            Lists.saveToList('podcasts','fav',_podcast)
  ...
  clickHandlerPodcastShare(event){
    event.stopPropagation();
  }
  componentDidMount(){
              ...
              this.state.data.map(p => (
                <div className="col-xs-6 col-md-3 col-lg-4" >
                  <div className={ p.id === localStorage.getItem('lastPodcast') ? "contentSelected" : "" } >
                      <div id={p.id} className="row item" name={p.name[localStorage.getItem('language')]} style={ 'background-image:url("' + p.image + '")'} onClick={() => this.props.initplayer.play(p.source, p.id, p.name)} >
                    ...
                    <div className="options" >
                      <div><div><img id='later' src={later} alt="later" onClick={ (event, _podcast) => this.clickHandlerPodcastLater(event, p.id) } /></div></div>
                      <div><div><img id='fav' src={fav} alt="fav" onClick={ (event, _podcast) => this.clickHandlerPodcastFav(event, p.id) } /></div></div>
                      <div><div><img id='share' src={share} alt="share" onClick={ this.clickHandlerPodcastShare } /></div></div>
                    </div>
                </div>
              ))
            }
